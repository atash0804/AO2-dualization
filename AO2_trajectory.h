...
protected:
    ...
    std::stack<ull*> not_cov_rows;

    std::stack<ull*> deleted_by_domination;
...
// choose the lightest column in lightest row
class AO2TrajectoryLL: public AO2MoptimizedTrajectory {
protected:
    ...
    virtual Element find_the_least() {
        ...
        coord least_d1 = -1;
        coord least_d2 = -1;
        coord least_weight = m+1, cur_weight, least_col;
        coord *columns = new coord[m]();
        ull tmp;
        ...
        for (coord i = 0; i < n; i++) {
            ...
            cur_weight = 0;
            for (coord j = 0; j < col_chunks; j++) {
                tmp = B[i][j]; // maybe change to M[i][j]
                while (tmp) {
                    tmp &= (tmp - 1);
                    cur_weight++;
                }
            }
            if (cur_weight && (cur_weight < least_weight)) {
                least_weight = cur_weight;
                least_col = n + 1;
                for (coord j = 0; j < m; j++) {
                    if (B[i][j/CH_SIZE] & (ull(1) << (CH_SIZE_1 - j % CH_SIZE)) && (columns[j] < least_col)) {
                        least_col = columns[j];
                        least_d2 = j;
                        for (coord i1 = 0; i1 < n; i1++) {
                            if (!B[i1]) continue;
                            if (B[i1][j / CH_SIZE] & (ull(1) << (CH_SIZE_1 - j % CH_SIZE))) {
                                least_d1 = i1;
                                break;
                            }
                        }
...
public:
    AO2TrajectoryLL(coord d1, coord d2, ull** Matrix) :
...
public:
    AO2TrajectoryBest(coord d1, coord d2, ull** Matrix) {
        n = d1;
        m = d2;
        col_chunks = (m-1) / CH_SIZE + 1;
        row_chunks = (n-1) / CH_SIZE + 1;
